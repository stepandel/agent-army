...
- [ ] Loop through `activeWork` array — for each item, process by state:
  ...
  - `BUILD_CHECK`: run `pnpm typecheck && pnpm build && pnpm test:e2e` in project dir. On pass: create PR, assign Scout as reviewer, move Linear ticket to "In Review by agent" or "In Review" and assign Scout, comment on the Linear ticket with the PR link, set `AWAITING_REVIEW`. On fail (< 3 attempts): spawn Claude Code to fix, set `AGENT_RUNNING`. On fail (3+ attempts): create draft PR with failure summary, comment on the Linear ticket with the draft PR link, set `AWAITING_REVIEW`.
...
- [ ] After processing all items: if `activeWork` has fewer than 2 items in `AGENT_RUNNING` or `BUILD_CHECK`, check Linear for next prioritized ticket assigned to you (Titus) that's in "To Do" or "In Backlog" state. If found: add to `activeWork` with state `AGENT_RUNNING`, set In Progress, spawn Claude Code with ticket context. If none: HEARTBEAT_OK. (Note: `AWAITING_REVIEW` items do NOT block picking up new work — only count `AGENT_RUNNING` and `BUILD_CHECK` toward the cap.)
