...
| `role` | string | Functional role identifier. Used for agent resolution (`agent-army ssh <role>`). Examples: `pm`, `eng`, `tester`, `researcher` |
...
| `description` | string | One-line summary of the agent's purpose. Shown during `agent-army init` and in status output. |
...
| `templateVars` | string[] | Template variable names used in workspace files (e.g., `SOUL.md`, `USER.md`). Values are collected during `agent-army init` and substituted at deploy time. |
...
Each entry provides an install script, model configuration, and OpenClaw `cliBackends` config. Defined in `cli/lib/coding-agent-registry.ts`.
...
Defined in `cli/lib/dep-registry.ts`.
...
Defined in `cli/lib/plugin-registry.ts`.
...
2. **Deployment overrides** — Plugin config files at `~/.agent-army/configs/<stack>/plugins/<plugin>.yaml`

## Identity vs Deployment Manifest

The `identity.yaml` and `agent-army.yaml` serve different purposes:

| Concern | `identity.yaml` | `agent-army.yaml` |
|---------|-----------------|-------------------|
...
Agent Army validates `identity.yaml` at deploy time. The following checks are enforced:
