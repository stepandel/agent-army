...
```
agent-army.yaml         Pulumi Stack          Cloud Provider
...
The dep registry (`cli/lib/dep-registry.ts`) maps each dep name to its install script.
...
The coding agent registry (`cli/lib/coding-agent-registry.ts`) maps each agent name to install and model configuration scripts.
