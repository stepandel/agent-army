...
description: "How Agent Army secures agent communication with Tailscale mesh VPN"
---

# Networking & Security

Agent Army uses [Tailscale](https://tailscale.com) to create a private mesh network between you and your agents. No ports are exposed to the public internet — all access flows through an encrypted WireGuard tunnel.
...
```
┌──────────┐    Tailscale     ┌──────────────────────────────────┐
│  You     │    WireGuard     │        Agent Server              │
│          │    tunnel        │                                  │
│  Browser ├──────────────────▶  Tailscale Serve (HTTPS :443)   │
│          │                  │       │                          │
│          │                  │       ▼                          │
...
│          │                  │       │                          │
│          │                  │       ▼                          │
│          │                  │  Shell session (ubuntu user)     │
└──────────┘                  └──────────────────────────────────┘

Inbound from public internet: ✗ blocked
Inbound from tailnet:         ✓ SSH + HTTPS only
Outbound from agent:          ✓ all (for GitHub, Linear, APIs)
```
