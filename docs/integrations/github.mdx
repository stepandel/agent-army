...
For Scout (QA), read-only access to Contents is sufficient if she only reviews PRs without pushing code.
...
|-------|-------------------|
| **Juno** (PM) | Monitors open PRs, checks CI status, reviews repo activity for progress tracking |
