...
1. Selects the existing Pulumi stack (or creates one if missing)
2. Syncs manifest and config to the Pulumi project
3. Cleans up stale Tailscale devices (cloud only)
4. Runs `pulumi up --refresh` to read actual cloud state and apply changes
5. Displays updated agent details (IPs, Tailscale URLs)
